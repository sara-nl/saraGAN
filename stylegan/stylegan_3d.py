...
class StyleGAN(object):
    ...
    def __init__(self, args):
        ...
        self.resolutions = resolution_list(self.img_size) # [4, 8, 16, 32, 64, 128, 256, 512, ...]
        ...
        self.g_learning_rates = {128: 0.0015, 256: 0.002, 512: 0.003}
        self.d_learning_rates = {128: 0.0015, 256: 0.002, 512: 0.003}

        self.print_freq = {4: 1000, 8: 1000, 16: 1000, 32: 1000, 64: 1000, 128: 3000, 256: 5000, 512: 10000}
        self.save_freq = {4: 1000, 8: 1000, 16: 1000, 32: 1000, 64: 1000, 128: 3000, 256: 5000, 512: 10000}
        ...
        self.datasets = load_data(data_dir=self.dataset_dir, 
        ...
        print("# scratch : ", self.scratch_dir)
    ...
    def g_mapping(self, z, n_broadcast):
    ...
    def g_synthesis(self, w_broadcasted, alpha, resolutions, featuremaps):
        ...
        x = synthesis_const_block(res, w_broadcasted, n_f)
        ...
        if self.progressive :
            ...
            for res, n_f in zip(resolutions[1:], featuremaps[1:]) :
                ...
                layer_index += 2
            images_out = torgb(x, resolutions[-1])

        return images_out

    def generator(self, z, alpha, target_img_size, is_training=True):
        resolutions = resolution_list(target_img_size)
        featuremaps = featuremap_list(target_img_size)
        
        w_avg = tf.Variable(name='w_avg', shape=[self.w_dim], 
                            dtype=tf.float32, initial_value=tf.zeros_initializer()(shape=[self.w_dim]), 
    ...
    def discriminator(self, x_init, alpha, target_img_size):
        resolutions = resolution_list(target_img_size)
        featuremaps = featuremap_list(target_img_size)
        ...
        """ set inputs """
        x = fromrgb(x_init, r_resolutions[0], r_featuremaps[0])
        ...
        for index, (res, n_f) in enumerate(zip(r_resolutions[:-1], r_featuremaps[:-1])):
            ...
            if self.progressive :
                x_init = downscale3d(x_init)
                y = fromrgb(x_init, res_next, n_f_next)
        ...
        logit = discriminator_last_block(x, res, n_f, n_f)
    ...
    def update_moving_average_of_w(self, w_broadcasted, w_avg):
        ...
        w_avg = lerp(batch_avg, w_avg, self.w_ema_decay)
    ...
    def style_mixing_regularization(self, z, w_broadcasted, n_broadcast, resolutions):
        with tf.name_scope('style_mix'):
            z2 = tf.random.normal(tf.shape(z), dtype=tf.float32)
            ...
            mixing_cutoff = tf.cond(tf.random.uniform([], 0.0, 1.0) < self.style_mixing_prob,
                lambda: tf.random.uniform([], 1, last_layer_index, dtype=tf.int32),
    ...
    def build_model(self):
        ...
        if self.phase == 'train' :
            ...
            for res in self.resolutions[self.resolutions.index(self.start_res):]:
                ...
                global_step = tf.Variable(name='global_step_{}'.format(res), shape=[], dtype=tf.float32, initial_value=tf.zeros_initializer()(shape=[]), trainable=False, aggregation=tf.VariableAggregation.ONLY_FIRST_REPLICA)
                ...
                #                         trainable=False, aggregation=tf.VariableAggregation.)
                alpha = tf.Variable(name='alpha_{}'.format(res), shape=[], dtype=tf.float32, initial_value=tf.zeros_initializer()(shape=[]) if do_train_trans else tf.ones_initializer()(shape=[]), trainable=False, aggregation=tf.VariableAggregation.ONLY_FIRST_REPLICA)
                ...
                dataset = self.datasets[res]
                real_img = next(iter(dataset.take(1)))
    ...
    def model_dir(self):
        ...
        return "{}_{}to{}{}".format(self.model_name, self.start_res, self.img_size, progressive)
