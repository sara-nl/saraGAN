...
def create_optimizer(loss, tvars, init_lr, num_train_steps, num_warmup_steps, hvd=None, manual_fp16=False, use_fp16=False,
                     num_accumulation_steps=1,
                     optimizer_type="adam", allreduce_post_accumulation=False, beta1=0.0, beta2=0.9):
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None,
        ...
        for (grad, param) in grads_and_vars:
            if grad is None or param is None:
                continue
            # grad = tf.Print(grad, [tf.norm(grad)], message='NaN Adam: ')
...
class LAMBOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def apply_gradients(self, grads_and_vars, global_step, name=None,
        ...
        for (grad, param) in grads_and_vars:
            ...
            # grad = tf.Print(grad, [hvd.rank(), self.beta_1, steps, global_step], 'HIER')
            param_name = self._get_variable_name(param.name)
            has_shadow = manual_fp16 and param.dtype.base_dtype != tf.float32
            if has_shadow:
                # create shadow fp32 weights for fp16 variable
            ...
            # next_m = tf.Print(m, [tf.norm(m), self.beta_1, tf.norm(grad)], 'm, g')

            # steps = tf.Print(steps, [steps], 'steps')
            # self.beta_1 = tf.Print(self.beta_1, [self.beta_1], 'beta1')


            beta1_correction = (1 - self.beta_1 ** steps)
            beta2_correction = (1 - self.beta_2 ** steps)

            # beta1_correction = tf.Print(beta1_correction, [beta1_correction], 'beta1')
            # next_m = tf.Print(next_m, [tf.norm(next_m)], 'next m')
            ...
            # g_norm = tf.Print(g_norm, [g_norm], 'norm')
            ...
            next_param = param_fp32 - update_with_lr
            # next_param = tf.Print(next_param, [hvd.rank(), tf.norm(next_m_unbiased), tf.norm(next_m), beta1_correction, self.beta_1, steps, global_step])
            next_param = tf.Print(next_param, [hvd.rank(), self.beta_1, steps, tf.constant([self.beta_1], dtype=tf.float32) ** steps])
