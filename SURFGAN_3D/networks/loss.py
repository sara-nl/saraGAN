...
def forward_generator(generator,
    ...
    gen_sample = generator(z, alpha, phase, num_phases,
                           base_dim, base_shape, activation=activation, kernel_shape=kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, 
    ...
    disc_fake_g = discriminator(gen_sample_noisy, alpha, phase, num_phases, base_shape, base_dim, latent_dim,
                                activation=activation, kernel_shape=kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, 
                                param=leakiness, size=network_size, is_reuse=is_reuse)
    if loss_fn == 'wgan':
...
def forward_discriminator(generator,
    ...
    disc_fake_d = discriminator(tf.stop_gradient(gen_sample_noisy), alpha, phase, num_phases,
                                base_shape, base_dim, latent_dim, activation=activation, kernel_shape=kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, 
                                param=leakiness, size=network_size, )
    disc_real = discriminator(real_image_input, alpha, phase, num_phases,
                              base_shape, base_dim, latent_dim, activation=activation, kernel_shape=kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, param=leakiness,
    ...
    gradients = tf.gradients(discriminator(interpolates, alpha, phase,
                                           ...
                                           is_reuse=True, activation=activation, kernel_shape=kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, 
...
def forward_simultaneous(generator,
    ...
    # Generator training.
    disc_fake_g = discriminator(gen_sample_noisy, alpha, phase, num_phases, base_shape, base_dim, latent_dim,
                                activation=activation, kernel_shape = kernel_shape, kernel_spec = kernel_spec, filter_spec = filter_spec, param=leakiness, size=network_size, is_reuse=True, conditioning=conditioning)
