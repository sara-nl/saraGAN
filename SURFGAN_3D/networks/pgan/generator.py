...
def generator_in(x, filters, shape, activation, kernel_shape, param=None):
...
def generator_block(x, filters_out, activation, kernel_shape, param=None):
...
def generator(x, alpha, phase, num_phases, base_dim, base_shape, activation, kernel_shape, param=None, size='medium', is_reuse=False, conditioning=None):
    ...
    with tf.variable_scope('generator') as scope:
        ...
        with tf.variable_scope('generator_in'):
            x = generator_in(x, filters=base_dim, shape=base_shape[1:], activation=activation, kernel_shape=kernel_shape, param=param)
        ...
        for i in range(2, phase + 1):
            ...
            with tf.variable_scope(f'generator_block_{i}'):
                x = generator_block(x, filters_out, activation=activation, kernel_shape=kernel_shape, param=param)
