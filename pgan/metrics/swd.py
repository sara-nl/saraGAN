...
def _laplacian_pyramid(batch, num_levels):
    ...
    def pyr_up(batch):    # matches cv2.pyrUp()
        ...
        res = array_ops.batch_to_space_nd(res, block_shape=[2, 2, 2], crops=[[0, 0], [0, 0], [0, 0]])
...
def _batch_to_patches(batch, patches_per_image, patch_size):
    ...
    def py_func_random_patches(batch):
        ...
        patch_id, z, y, x, chan = np.ogrid[0:patch_count, -hs:hs + 1, -hs:hs + 1, -hs: hs + 1, 0:channels]
        img_id = patch_id // patches_per_image
        # pylint: disable=g-no-augmented-assignment
        # Need explicit addition for broadcast to work properly.
        z = z + np.random.randint(hs, depth - hs, size=(patch_count, 1, 1, 1, 1))
        y = y + np.random.randint(hs, height - hs, size=(patch_count, 1, 1, 1, 1))
        x = x + np.random.randint(hs, width - hs, size=(patch_count, 1, 1, 1, 1))
...
def sliced_wasserstein_distance_3d(real_images,
        ...
        Raises:
                        ...
                        same size.
        """
        depth = real_images.shape[1]
        # Select resolutions.
        resolution_full = int(depth)
        ...
        for lod in range(len(resolutions)):
            if not use_svd:
                scores.append((_sliced_wasserstein(patches_real[lod], patches_test[lod],
